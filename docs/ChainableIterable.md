...
const a = ChainableIterable.repeatIterable(3, oneTime(3))
...
const a = ChainableIterable.repeatIterable(3, fn(3))
...
const a = ChainableIterable.from(input).flattenPerFunction(x => ({ iterate: isIterable(x), itemToYield: x })).toArray()
